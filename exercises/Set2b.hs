...
oddFactorial n = oddFactorial' 1 1
  where
    oddFactorial' curr acc
        | curr > n = acc
        | otherwise = oddFactorial' (curr + 2) (curr * acc)
...
myGcd a 0 = a
myGcd a b = myGcd small (mod big small)
  where
...
leftpad str count = leftpad' str (count - length str)
  where
    leftpad' str 0 = str
    leftpad' str rem = leftpad' (' ' : str) (rem - 1)
...
smallestDivisor n = smallestDivisor' 2
  where
    smallestDivisor' divisor = case mod n divisor of
        0 -> divisor
        _ -> smallestDivisor' (divisor + 1)
