...
longest =
    foldr1
        ( \a b -> case (compareF length a b, compareF head a b) of
            (GT, _) -> a
            (LT, _) -> b
            (EQ, LT) -> a
            (EQ, _) -> b
  ...
  where
    compareF f a b = compare (f a) (f b)

-- longest (xs : xs' : xss) = case compare (length xs) (length xs') of
--     GT -> longest (xs : xss)
--     LT -> longest (xs' : xss)
--     EQ -> longest ((if head xs < head xs' then xs else xs') : xss)
-- longest [xs] = xs
-- longest [] = error "unreachable"
...
incrementKey key = map (\(k, v) -> (k, v + if k == key then 1 else 0))
...
freqs = foldr (Map.alter (Just . (+ 1) . fromMaybe 0)) Map.empty
...
transfer from to amount bank =
    case (Map.lookup from bank, Map.member to bank) of
        (Just bal, True)
            | amount > 0 && bal >= amount ->
                transfer (+) to . transfer (-) from $ bank
        _ -> bank
  where
    transfer op = Map.adjust (flip op amount)
...
maxIndex = fst . maximumBy (\e e' -> compare (snd e) (snd e')) . assocs
