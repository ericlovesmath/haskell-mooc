...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix color color') = zipWith (\c c' -> (c + c') / 2) (rgb color) (rgb color')
...
toNat n
    | n < 0 = Nothing
    | otherwise = Just (toNat' n Zero)
  where
    toNat' 0 nat = nat
    toNat' n nat = toNat' (n - 1) (PlusOne nat)
...
toBin 0 = O End
toBin 1 = I End
toBin n = (if even n then O else I) (toBin (div n 2))
-- toBin 0 = O End
