...
instance Eq a => Eq (List a) where
    Empty == Empty = True
    LNode val rem == LNode val' rem' = val == val' && rem == rem'
    _ == _ = False
...
simplify (RationalNumber p q) = RationalNumber (div p f) (div q f)
  where
    f = gcd p q
...
instance Num RationalNumber where
    ...
    signum (RationalNumber a _) = case compare a 0 of
        GT -> 1
        EQ -> 0
        LT -> -1
