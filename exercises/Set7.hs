...
add x (Set xs)
    ...
    | otherwise = Set (add' x xs)
  where
    add' x [] = [x]
    add' x (hd : tl) = if x < hd then x : hd : tl else hd : add' x tl
...
data State = Start | NeedFlourSugar | NeedFlour | NeedSugar | Prepped | Mixed | Error | Finished
    deriving (Eq, Show)

step :: State -> Event -> State
step Start AddEggs = NeedFlourSugar
step NeedFlourSugar AddFlour = NeedSugar
step NeedFlourSugar AddSugar = NeedFlour
step NeedFlour AddFlour = Prepped
step NeedSugar AddSugar = Prepped
step Prepped Mix = Mixed
...
reverseNonEmpty (x :| []) = x :| []
reverseNonEmpty (x :| xs) = let (a : as) = reverse (x : xs) in a :| as
...
passwordAllowed pw (ContainsSome str) = any (flip elem pw) str
passwordAllowed pw (DoesNotContain str) = not $ passwordAllowed pw (ContainsSome str)
passwordAllowed pw (And req req') = passwordAllowed pw req && passwordAllowed pw req'
passwordAllowed pw (Or req req') = passwordAllowed pw req || passwordAllowed pw req'
...
operation "*" x y = Multiply x y
operation _ _ _ = error "Unexpected operation passed to operation"
