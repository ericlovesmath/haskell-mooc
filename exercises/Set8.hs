...
combine combiner (Picture pic) (Picture pic') = Picture (\coord -> combiner (pic coord) (pic' coord))
...
paintSolid color (Shape isInShape) (Picture base) = Picture f
  where
    f coord
        | isInShape coord = color
...
instance Transform Blur where
    apply Blur (Picture f) = Picture f'
      where
        f' = divColor 5 . foldr1 addColor . map f . neighbors
        neighbors (Coord x y) = [Coord (x - 1) y, Coord (x + 1) y, Coord x (y - 1), Coord x (y + 1), Coord x y]
        addColor (Color r g b) (Color r' g' b') = Color (r + r') (g + g') (b + b')
        divColor n (Color r g b) = Color (div r n) (div g n) (div b n)
