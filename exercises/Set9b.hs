...
prettyPrint n queens = pp (1, 1) (sort queens)
  where
    pp coord queens
        | fst coord > n = ""
        | snd coord > n = '\n' : pp (nextRow coord) queens
        | queens /= [] && coord == head queens = 'Q' : pp (nextCol coord) (tail queens)
        | otherwise = '.' : pp (nextCol coord) queens
...
queenSight :: Coord -> Coord -> Bool
queenSight square queen = any (\f -> f square queen) [sameRow, sameCol, sameDiag, sameAntidiag]

danger :: Candidate -> Stack -> Bool
danger square queens = any (queenSight square) queens
...
prettyPrint2 n queens = pp (1, 1)
  where
    pp (x, y)
        | x > n = ""
        | y > n = '\n' : pp (nextRow (x, y))
        | elem (x, y) queens = 'Q' : pp (nextCol (x, y))
        | danger (x, y) queens = '#' : pp (nextCol (x, y))
        | otherwise = '.' : pp (nextCol (x, y))
...
fixFirst n [] = error "Expected Stack of Queens in fixFirst"
fixFirst n (queen : queens)
    | snd queen > n = Nothing
    | danger queen queens = fixFirst n (nextCol queen : queens)
    | otherwise = Just (queen : queens)
--------------------------------------------------------------------------------
...
continue [] = [(1, 1)]
continue (e : s) = nextRow e : e : s

backtrack :: Stack -> Stack
backtrack (_ : e : s) = nextCol e : s
backtrack _ = error "Attempted to backtrack empty stack"
